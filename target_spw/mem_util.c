...
//for write buffer bitmap

void _set_bit_dram_more_16k(UINT32 const base_addr, UINT32 const bit_offset , UINT32 const num_bits)
{
	if(!num_bits)
		return ;
	ASSERT(base_addr >= DRAM_BASE && base_addr < (DRAM_BASE + DRAM_SIZE));
	UINT32 temp = 0xFFFFFFFF ;
	UINT32 temp1 = 0xFFFFFFFF ;
	UINT32 val;
	...
	//uart_printf("val : %x" , val);
	temp &= (temp << new_bit_offset);
	//uart_printf("temp : %x" , temp);
	temp1 &= (temp1 >> (32 - num_bits - new_bit_offset));
	//uart_printf("temp1 : %x" , temp1);
	temp &= temp1 ;
	//uart_printf("temp : %x" , temp);
	val |= temp ;
	//uart_printf("val : %x" , val);
...
void _set_bit_dram_more(UINT32 const base_addr, UINT32 const bit_offset , UINT32 const num_bits)
{
	
	
	#if OPTION_2_PLANE == 1
	UINT32 bit_offset_low , bit_offset_high ; 
	UINT32 bit_num_bits_low , bit_num_bits_high ; 
	if(bit_offset < 32)
		{
			bit_offset_low = bit_offset ;
			bit_offset_high = 0 ;
			bit_num_bits_low = (num_bits >= (32 - bit_offset)) ? (32 - bit_offset) : (num_bits);
			bit_num_bits_high = num_bits - bit_num_bits_low;
		}
	else
		{
			bit_offset_low = 0 ;
			bit_num_bits_low = 0; 
			bit_offset_high = bit_offset - 32 ;
			bit_num_bits_high = num_bits ; 
			
		}
	
	
	_set_bit_dram_more_16k(base_addr, bit_offset_low, bit_num_bits_low);
	_set_bit_dram_more_16k(base_addr+4, bit_offset_high,bit_num_bits_high);
	#else
		_set_bit_dram_more_16k(base_addr, bit_offset, num_bits);
...
void _clr_bit_dram_16k_all(UINT32 const base_addr)
{
	ASSERT(base_addr >= DRAM_BASE && base_addr < (DRAM_BASE + DRAM_SIZE));

	UINT32 val , bit_offset = 0;
	UINT32 addr = base_addr + bit_offset / 8;
	UINT32 aligned_addr = addr & 0xFFFFFFFC;
	UINT32 new_bit_offset = 0xFFFFFFFF;
	UINT32 real_addr = DRAM_BASE + (aligned_addr - DRAM_BASE)/128*132 + (aligned_addr - DRAM_BASE)%128;

	val = *(volatile UINT32*)real_addr;
	val &= ~(new_bit_offset);
	//uart_printf("clean val : %d" , val);
	mem_copy(aligned_addr, &val, sizeof(UINT32));
}
UINT32 _read_bit_dram_16k_all(UINT32 const base_addr)
{
	ASSERT(base_addr >= DRAM_BASE && base_addr < (DRAM_BASE + DRAM_SIZE));
...
void _change_bit_dram_16k_all(UINT32 const base_addr1 ,UINT32 const base_addr2 )
{
	UINT32 val1 = _read_bit_dram_16k_all(base_addr1);
	UINT32 val2 = _read_bit_dram_16k_all(base_addr2);

	UINT32 aligned_addr = base_addr2 & 0xFFFFFFFC;
	UINT32 new_bit_offset = 0xFFFFFFFF;
	UINT32 real_addr = DRAM_BASE + (aligned_addr - DRAM_BASE)/128*132 + (aligned_addr - DRAM_BASE)%128;

	mem_copy(aligned_addr, &val1, sizeof(UINT32));
	aligned_addr = base_addr1 & 0xFFFFFFFC;
	new_bit_offset = 0xFFFFFFFF;
	real_addr = DRAM_BASE + (aligned_addr - DRAM_BASE)/128*132 + (aligned_addr - DRAM_BASE)%128;
	mem_copy(aligned_addr, &val2, sizeof(UINT32));
}

void _clr_bit_dram_all(UINT32 const base_addr)
{

	#if OPTION_2_PLANE
		_clr_bit_dram_16k_all(base_addr);
		_clr_bit_dram_16k_all(base_addr+4);
	#else
		 _clr_bit_dram_16k_all(base_addr);
...
BOOL32 _tst_bit_dram_more_16k(UINT32 const base_addr)//, UINT32 const bit_offset)
{
	ASSERT(base_addr >= DRAM_BASE && base_addr < (DRAM_BASE + DRAM_SIZE));
	UINT32 bit_offset = 0 ; 
	UINT32 val;
...
BOOL32 _tst_bit_dram_more(UINT32 const base_addr)//, UINT32 const bit_offset)
{


#if OPTION_2_PLANE
	return  (_tst_bit_dram_more_16k(base_addr) && _tst_bit_dram_more_16k(base_addr+4)) ;
#else
    return 	_tst_bit_dram_more_16k(base_addr) ;
#endif
...
void _set_bit_dram(UINT32 const base_addr, UINT32 const bit_offset)
{
	...
	UINT32 val;
	UINT32 addr = base_addr + bit_offset / 8; //byte addr?
	UINT32 aligned_addr = addr & 0xFFFFFFFC;
	UINT32 new_bit_offset = (base_addr*8 + bit_offset) % 32;
	UINT32 real_addr = DRAM_BASE + (aligned_addr - DRAM_BASE)/128*132 + (aligned_addr - DRAM_BASE)%128;
